...
final class Block {
	...
	public static function register_blocks() {
		...
		$blocks = apply_filters( 'woorricane_helper_gutenberg_blocks', array() );
	...
	public static function register_admin( $block = '', $args = array() ) {
		...
		wp_register_script(
			'woorricane-helper-' . $block_name . '-blocks',
	...
	public static function register( $block = '', $args = array() ) {
		...
		// Set editor styles.
		$editor_style = 'woorricane-helper-' . $block_name . '-blocks';
		if ( ! empty( $args['editor_style'] ) ) {
			$editor_style = $args['editor_style'];
		}

		register_block_type(
			'woorricane-helper/' . $block_name,
			array(
				'style'         => $styles,
				'script'        => $script,
				'editor_style'  => $editor_style,
				'editor_script' => 'woorricane-helper-' . $block_name . '-blocks',
			)
		);
	}
}
